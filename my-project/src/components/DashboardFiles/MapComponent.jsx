...
  // Helper to clear the Mapbox pop-up (for hover)
...
    removeMapboxPopup(); 
    clearManholeSelection(); // Clear any selected manhole
...
    // --- Event Listeners (Back to original state-setting logic) ---
...
      selectedManholeIdRef.current = clickedManholeId;
      // THIS IS THE KEY: Just set the state. The JSX will handle rendering the component.
...
  removeWardLayer();
  ...
  // Also remove manhole selection when changing wards
  clearManholeSelection(); 
...
    if (map.current) {
      // Re-centered the map to a more general view
      map.current.flyTo({ center: [78.4794, 17.3940], zoom: 9.40, });
    }
  };
...
      
      {/* --- Left section: Controls + Map (Static 70% width) --- */}
    
...
    		className="map-box relative rounded-lg overflow-hidden border  border-gray-300"
            // TYPO REMOVED HERE
    		style={{ height: "445.52px", opacity: 1 }} // This height matches your original
    	  >
    		{/* Reset Button */}
